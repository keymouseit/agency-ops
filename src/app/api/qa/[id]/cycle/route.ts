...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (project) {
    logger.debug('Sending QA test cycle notifications', { projectId: params.id, projectName: project.name, result: data.result, ownerId: project.ownerId })

    if (data.result === 'fail') {
      // Notify the dev project owner: their project is blocked in QA
      logger.info('Sending test_cycle_fail notification to project owner', { projectId: params.id, ownerId: project.ownerId })
      await notify('test_cycle_fail', [project.ownerId],
        `QA: ${project.name} is blocked — ${(data.blockerNote ?? 'see test report').slice(0, 80)}`,
        `/qa/${params.id}`)
      logger.info('test_cycle_fail notification sent', { projectId: params.id, ownerId: project.ownerId })
      ...
      logger.info('Sending test_cycle_pass notification to project owner', { projectId: params.id, ownerId: project.ownerId, label })
      await notify('test_cycle_pass', [project.ownerId],
        `QA: ${project.name} test cycle ${label} — great work!`,
        `/qa/${params.id}`)
      logger.info('test_cycle_pass notification sent to project owner', { projectId: params.id, ownerId: project.ownerId })
